...
Ao final de uma sessão bem-sucedida, chame `wiki_upsert_page` com `type: "feature"` e `status: "discovery"`.

Seções do `body` a preencher com os aprendizados da sessão:
...
Registre no log chamando `wiki_log_append`:
```json
{ "type": "discovery", "summary": "<nome da feature>", "details": "Fase: em discovery | validado. Hipótese: <resumo>. Experimento: <como validar ou \"dispensado — motivo\">." }
```
